// Vendored from apps/frontend/src/entities/cityMap/theme/themeRepository.ts
// Ported from Terraink (features/theme/infrastructure/themeRepository.ts).
// NOTE: additional_themes.json exists but is currently unused.
...
const themeColorLookup: Record<ThemeColorKey, string[]> = {
  ...
  'map.roads.minor_high': ['map.roads.minor_high', 'map.roads.minor', 'road_primary', 'road_secondary'],
  'map.roads.minor_mid': ['map.roads.minor_mid', 'map.roads.minor', 'road_secondary', 'road_tertiary'],
  'map.roads.minor_low': ['map.roads.minor_low', 'map.roads.minor', 'road_residential', 'road_default'],
...
function normalizeTheme(themeInput: unknown): ResolvedTheme {
  ...
  const description = String(getPathValue(theme, 'description') ?? '').trim() || fallbackTheme.description;
  ...
  const land = resolveByCandidates(theme, themeColorLookup['map.land']) || uiBg || fallbackTheme.map.land;
  const water = resolveByCandidates(theme, themeColorLookup['map.water']) || fallbackTheme.map.water;
  const waterway = resolveByCandidates(theme, themeColorLookup['map.waterway']) || water;
  const parks = resolveByCandidates(theme, themeColorLookup['map.parks']) || fallbackTheme.map.parks;
  const landcover = resolveByCandidates(theme, themeColorLookup['map.landcover']) || blendHex(land, parks, 0.35);

  const roadMajor = resolveByCandidates(theme, themeColorLookup['map.roads.major']) || uiText;
  const roadMinorHigh = resolveByCandidates(theme, themeColorLookup['map.roads.minor_high']) || roadMajor;
  const roadMinorMid = resolveByCandidates(theme, themeColorLookup['map.roads.minor_mid']) || roadMinorHigh;
  const roadMinorLow = resolveByCandidates(theme, themeColorLookup['map.roads.minor_low']) || blendHex(roadMinorMid, land, 0.28);
  const roadPath = resolveByCandidates(theme, themeColorLookup['map.roads.path']) || roadMinorLow;
  const roadOutline = resolveByCandidates(theme, themeColorLookup['map.roads.outline']) || blendHex(land, uiText, 0.12);

  const buildings = resolveByCandidates(theme, themeColorLookup['map.buildings']) || blendHex(land, uiText, 0.14);
  const aeroway = resolveByCandidates(theme, themeColorLookup['map.aeroway']) || blendHex(land, water, 0.2);
...
const manifestThemes = isObject(themesManifest) ? (themesManifest as ThemeObject).themes : undefined;
...
const themesByName: Record<string, ThemeObject> = Object.entries(rawThemes).reduce(
  (acc: Record<string, ThemeObject>, [key, value]) => {
    if (key && isObject(value)) acc[key] = value;
    return acc;
  },
  {}
);
...
export function getThemePalette(theme: unknown): string[] {
  ...
  return DISPLAY_PALETTE_KEYS.map((key: ThemeColorKey) =>
    String(getThemeColorByPath(normalizedTheme, key) ?? '').trim()
  ).filter((color: string) => isCssColor(color));
}

export const themeOptions: ThemeOption[] = themeNames.map((name: string) => ({
  id: name,
...
const preferredDefaultThemeName = 'classic_night';
...
export function getTheme(themeName: string): ResolvedTheme {
  if (themesByName[themeName]) return normalizeTheme(themesByName[themeName]);
  if (defaultThemeName && themesByName[defaultThemeName]) return normalizeTheme(themesByName[defaultThemeName]);
